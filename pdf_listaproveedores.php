...
$this->Cell(3,0.5,utf8_decode('   Código'),1,0,'L',1);
$this->Cell(13.6,0.5,utf8_decode('   Descripción Producto'),1,0,'L',1);
$this->Cell(3,0.5,'   Precio Venta',1,1,'L',1);
$this->SetTextColor(0);
$this->Cell(0,0.1,'',1,1,'');
...
$pdf->Cell(6.97,0.5,'',0,0,'');
$fecha=date('d - m - Y');
$pdf->Cell(8.5,0.5,'LISTADO DE PROVEEDORES /  Fecha: '.$fecha.'.',1,1,'');
...
$consulta_sql="SELECT nombre, mail, telefono, direccion FROM proveedor ORDER BY nombre;";
...
$pdf->Cell(6.6,0.5,utf8_decode('Nombre'),1,0,'L',1);
$pdf->Cell(5,0.5,utf8_decode('Correo'),1,0,'L',1);
$pdf->Cell(2,0.5,utf8_decode('Teléfono'),1,0,'L',1);
$pdf->Cell(6,0.5,utf8_decode('Dirección'),1,1,'L',1);
$pdf->SetTextColor(0);
$pdf->Cell(0,0.1,'',1,1,'');
$i=0;


while($datos_proveedor=mysql_fetch_array($resultado)){
	if($i%2==0){
...
$pdf->Row(array($datos_proveedor['nombre'],utf8_decode($datos_proveedor['mail']), $datos_proveedor['telefono'],utf8_decode($datos_proveedor['direccion'])));
$i++;
}
$pdf->ln();
