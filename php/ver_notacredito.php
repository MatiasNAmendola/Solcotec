<?
$resultado2=mysql_query("select * from nota_credito where id_fv='$id_factura';") or die (mysql_error()); //captura todos los datos de nota de credito.
$datos_nota=mysql_fetch_array($resultado2);
?>

<form method="post">
<h1>Nota de Crédito Emitida [ <strong style="color:#CC0000">FOLIO Nº <? echo $datos_nota["folio"] ?></strong> ]</h1>
<h2>A continuación se muestra el detalle de la nota de crédito emitida.</h2>
<textarea style="margin-bottom:0em;"  name="comentario_notacredito" class="supertextarea" ><? echo $datos_nota["comentario_notacredito"]?></textarea>
...
$id_cliente=$datos_nota["id_cliente"];
...
$resultado4=mysql_query("select producto.codigo_producto,notacredito_x_producto.detalle, notacredito_x_producto.cantidad, notacredito_x_producto.precio_venta, notacredito_x_producto.precio_final, notacredito_x_producto.descuento, notacredito_x_producto.id_producto, notacredito_x_producto.detalle, notacredito_x_producto.id_fv   from notacredito_x_producto , producto where notacredito_x_producto.id_fv='$id_factura' AND producto.id_producto=notacredito_x_producto.id_producto;") or die ("fallo la mega consulta nota");
...
<?
$resultado=mysql_query("select nombre from vendedor where nombre != '$nombre_vendedor'") or die("error al obtener nombre vendedor");
...
<? $tipo_documento=$datos_nota["tipo_documento"];
...
<div class="ingreso">
<input type="text" id="doc" name="documento_rel" placeholder="# del documento" class="campo" value="<? echo $datos_nota["documento_rel"]?>"/>
</div>
...
<textarea id="comentario_factura" name="comentario_factura" class="supertextarea" placeholder="ingrese un comentario sobre la factura."><? echo $datos_nota["comentario_factura"]?></textarea>
