...
if($_SESSION['tipo_usuario']!=1){
	...
	$resultado=mysql_query("select id_producto, codigo_producto, nombre, stock, neto, p_venta, posicion  from producto where stock !=0 AND stock>0;") or die (mysql_error());
...
echo"<p>Hay $numero tipos de productos ingresados en bodega.</p>";
	...
	$variable2='<th colspan="2"></th>';
		...
		$resultado=mysql_query("select codigo_producto, nombre, stock, p_venta, posicion  from producto where stock !=0 AND stock>0;") or die (mysql_error());
$numero = mysql_num_rows($resultado);
echo"<p>Hay $numero tipos de productos ingresados en bodega.</p>";
		}	
...
<form method="post" style="float:right" action="consulta_producto.php" enctype="application/x-www-form-urlencoded">
<input type="hidden" name="caca" value="0"/>
<input type="submit" id="boton_stocks" class="boton" value="Consultar Todos">
</form>
