...
$pdf->Cell(6.97,0.5,'',0,0,'');
$fecha=date('d - m - Y');
$pdf->Cell(7.5,0.5,'LISTADO DE PRECIOS  /  Fecha: '.$fecha.'.',1,1,'');
...
$consulta_sql="SELECT codigo_producto, nombre, p_venta FROM producto ORDER BY marca, nombre;";
...
$pdf->Cell(3,0.5,utf8_decode('   Código'),1,0,'L',1);
$pdf->Cell(13.6,0.5,utf8_decode('   Descripción Producto'),1,0,'L',1);
$pdf->Cell(3,0.5,'   Precio Venta',1,1,'L',1);
$pdf->SetTextColor(0);
$pdf->Cell(0,0.1,'',1,1,'');
$i=0;
...
$pdf->Row(array('   '.$datos_producto['codigo_producto'],'   '.utf8_decode($datos_producto['nombre']), '   '.$datos_producto['p_venta']));
$i++;
}
$pdf->ln();
